...
class MountainCarAgent(object):
	...
	def __init__(self, world, tile_shape, ntiles, lamb):
		...
		self.ntiles = ntiles
		self.x_tilewidth = (self.x_bounds[1] - self.x_bounds[0])/(tile_shape[1]-1)
		self.v_tilewidth = (self.v_bounds[1] - self.v_bounds[0])/(tile_shape[0]-1)
	...
	def get_features(self,pos,vel,action):
		...
		for i in range(self.ntiles):
			...
			t2 = np.int(np.floor((self.x-self.x_bounds[0]+self.tile_offsets[i,0])/self.x_tilewidth))
			t3 = np.int(np.floor((self.v-self.v_bounds[0]+self.tile_offsets[i,1])/self.v_tilewidth))
